...
static bool dump_graph(Graph &g, const char *filename) {
    ...
    fwrite(g.vertex, sizeof(e_index_t), g.v_cnt + 1, fp);
    fwrite(g.edge, sizeof(v_index_t), g.e_cnt, fp);
    fclose(fp);
    return true;
}
...
static bool load_graph(Graph &g, const char *filename) {
    ...
    g.vertex = new e_index_t[h.v_cnt + 1];
    g.edge = new v_index_t[g.e_cnt];

    if (fread(g.vertex, sizeof(e_index_t), h.v_cnt + 1, fp) != h.v_cnt + 1) {
        printf("load_graph: failed to load vertexes.\n");
        return false;
    }
    if (fread(g.edge, sizeof(v_index_t), h.e_cnt, fp) != h.e_cnt) {
        printf("load_graph: failed to load edges.\n");
        return false;
    }
    printf("load_graph: %u vertexes, %lu edges\n", g.v_cnt, g.e_cnt);
    return true;
}
...
bool DataLoader::load_data(Graph *&g, DataType type, const char *path, bool binary_input, int oriented_type) {
    if (type == Patents || type == Orkut || type == complete8 || type == LiveJournal || type == MiCo || type == CiteSeer || type == Wiki_Vote ||
        type == YouTube || type == Friendster || type == skitter) {
...
bool DataLoader::load_labeled_data(LabeledGraph *&g, DataType type, const char *path) {
    if (type == Patents || type == Orkut || type == complete8 || type == LiveJournal || type == MiCo || type == CiteSeer || type == Wiki_Vote ||
        type == Twitter || type == YouTube || type == Friendster) {
...
static inline bool read_i32(FILE *fp, bool binary, int32_t &u) {
    if (!binary) {
        return fscanf(fp, "%d", &u) == 1;
    } else {
        bool f1 = (fread(&u, sizeof(int32_t), 1, fp) == 1);
        return f1;
    }
}
...
bool DataLoader::general_load_data(Graph *&g, DataType type, const char *path, bool binary, int oriented_type) {
    ...
    case DataType::skitter: {
        g->tri_cnt = skitter_tri_cnt;
        break;
    }
    default: {
        g->tri_cnt = -1;
    ...
    if (oriented_type != 0) {
        ...
        if (oriented_type == 2)
            std::sort(rank, rank + g->v_cnt, cmp_degree_lt);
    ...
    delete[] e;
    printf("Success! There are %d nodes and %lu edges.\n", g->v_cnt, g->e_cnt);
    fflush(stdout);
...
bool DataLoader::twitter_load_data(Graph *&g, DataType type, const char *path, int oriented_type) {
    ...
    g->edge = new v_index_t[g->e_cnt];
    g->vertex = new e_index_t[g->v_cnt + 1];
    for (v_index_t i = 0; i < g->v_cnt + 1; ++i)
...
bool DataLoader::general_load_labeled_data(LabeledGraph *&g, DataType type, const char *path) {
    ...
    std::sort(label_deg.begin(), label_deg.end(),
    ...
    for (int i = 0; i < g->v_cnt; ++i)
        rank[i] = std::make_tuple(g->v_label[i], -degree[i], i);
...
bool DataLoader::cmp_pair(std::pair<int, int> a, std::pair<int, int> b) { return a.first < b.first || (a.first == b.first && a.second < b.second); }
...
bool DataLoader::cmp_label(std::pair<int, int> a, std::pair<int, int> b) { return a.second < b.second; }

bool DataLoader::cmp_degree_gt(std::pair<int, int> a, std::pair<int, int> b) { return a.second > b.second; }

bool DataLoader::cmp_degree_lt(std::pair<int, int> a, std::pair<int, int> b) { return a.second < b.second; }
