...
int Graphmpi::getdegree() { return length[omp_get_thread_num()]; }

bool Graphmpi::include(int u) { return mynodel <= u && u < mynoder; }

Graphmpi::Graphmpi() {}

Graphmpi::~Graphmpi() { MPI_Finalize(); }
...
std::pair<int, int> Graphmpi::get_vertex_range() {
    ...
    global_vertex_lock.clear();
    if (returnflag)
        return std::make_pair(tmpvertex, std::min(mynoder, tmpvertex + chunksize));
    int thread_num = omp_get_thread_num();
#pragma omp critical
