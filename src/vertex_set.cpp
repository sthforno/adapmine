...
void VertexSet::build_vertex_set_bs(const Schedule_IEP &schedule, const VertexSet *vertex_set, Bitmap *bs, int *input_data, int input_size,
...
// 输入第一个为父亲节点的候选解，第二个为当前处理顶点的邻居集合， 第三个为当前处理顶点
...
// 输入第三四项为当前处理顶点的边表大小，第五项为需要处理的每一个模式id，第六项为当前匹配的数据顶点
void VertexSet::build_vertex_set(const Schedule_IEP &schedule, const VertexSet *vertex_set, int *input_data, int input_size, int prefix_id,
                                 int min_vertex, bool clique) {
    int father_id = schedule.get_father_prefix_id(prefix_id);
    if (father_id == -1) // 如果是最后一个顶点了，直接初始化即可
        init(input_size, input_data);
    else {
        init();
        VertexSet tmp_vset;
        tmp_vset.init(input_size, input_data);
        intersection(vertex_set[father_id], tmp_vset, min_vertex, clique); // 当前处理顶点的边集大小与父亲节点的边集去做交集，好像没用
    }
}

void VertexSet::build_vertex_set_bs_only_size(const Schedule_IEP &schedule, const VertexSet *vertex_set, Bitmap *bs, int *input_data, int input_size,
                                              int prefix_id, int depth) {
    int father_id = schedule.get_father_prefix_id(prefix_id);
    // assert (father_id != -1);
    size = 0;
