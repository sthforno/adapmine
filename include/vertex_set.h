...
struct Bitmap {
    uint8_t *s;
    int size;
    Bitmap(int _size) : s(nullptr) {
        s = new uint8_t[size = _size];
        set_0();
    }
    ~Bitmap() { delete[] s; }
...
class VertexSet {
  public:
    ...
    void init_bs(Bitmap *bs, int input_size, int *input_data);
    ...
    void intersection_bs(const VertexSet &set0, Bitmap *bs, int *input_data, int input_size, int depth);

    void intersection_with(const VertexSet &set1);
    void intersection_bs_only_size(const VertexSet &set0, Bitmap *bs, const VertexSet &set1, int depth);
    ...
    void build_vertex_set(const Schedule_IEP &schedule, const VertexSet *vertex_set, int *input_data, int input_size, int prefix_id,
                          int min_vertex = -1, bool clique = false);
    void build_vertex_set_bs(const Schedule_IEP &schedule, const VertexSet *vertex_set, Bitmap *bs, int *input_data, int input_size, int prefix_id,
                             int depth);
    void build_vertex_set_bs_only_size(const Schedule_IEP &schedule, const VertexSet *vertex_set, Bitmap *bs, int *input_data, int input_size,
                                       int prefix_id, int depth);
