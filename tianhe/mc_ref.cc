...
bool contains(const std::vector<int> &v, int vt) { return contains(&v[0], v.size(), vt); }
...
struct NeighborSet {
    ...
    bool contains(int vt) const { return my::contains(&g.edge[g.vertex[v]], size(), vt); }
};
    ...
    for (int v0 = 0; v0 < V(g); ++v0) {
        ...
        for (int v1 : s12) {
            ...
            for (int v2 : s12) {
                ...
                auto s3_1 = N(g, v2);
                std::vector<int> s3;
                std::set_intersection(s3_0.begin(), s3_0.end(), s3_1.begin(), s3_1.end(), std::back_inserter(s3));
                for (int v3 : s3) {
                    if (v3 >= v0)
                        break;
...
#pragma omp parallel for reduction(+ : res)
    for (int v0 = 0; v0 < V(g); ++v0) {
        ...
        for (int v1 : s1) {
            ...
            auto s23_0 = N(g, v1);
            std::vector<int> s23;
            std::set_intersection(s1.begin(), s1.end(), s23_0.begin(), s23_0.end(), std::back_inserter(s23));
            for (int v2 : s23) {
                for (int v3 : s23) {
                    if (v3 >= v2)
                        break;
...
#pragma omp parallel for reduction(+ : res)
    for (int v0 = 0; v0 < V(g); ++v0) {
        ...
        for (int v1 : s13) {
            auto s2_0 = N(g, v1);
            std::vector<int> s2;
            std::set_intersection(s13.begin(), s13.end(), s2_0.begin(), s2_0.end(), std::back_inserter(s2));
            for (int v2 : s2) {
                if (v2 >= v1)
                    break;
